...
async def search_inventory(agent_id: str, args: dict):
    df = await _get_inventory_df(agent_id)
    if df is None:
        return "Error técnico o inventario vacío."
    ...
    try:
        ...
        if "tipo_operacion" in results.columns:
            ...
            results = results[
                results["tipo_operacion"]
                .astype(str)
                .apply(normalize_text)
                .str.contains(op_norm, na=False)
            ]
        ...
        if args.get("tipo_inmueble") and "tipo_inmueble" in results.columns:
            ...
            match_key = tipo
            if "apto" in tipo or "apartamento" in tipo:
                ...
                match_key = "bodega"
            elif "oficina" in tipo or "consultorio" in tipo:
                match_key = "oficina"

            results = results[
                results["tipo_inmueble"]
                .astype(str)
                .apply(normalize_text)
                .str.contains(match_key, na=False)
            ]
            ...
            try:
                ...
                if op.lower() == "arriendo":
                    if "canon_mensual_cop" in results.columns:
                        ...
                        admin = pd.to_numeric(
                            results.get("valor_admin_cop", 0), errors="coerce"
                        ).fillna(0)
        ...
        if results.empty:
            return f"No encontré propiedades en {op} con esos criterios."
